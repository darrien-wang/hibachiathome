...
**演与不演的界线（用户 09-18 定）**：姿态可以演——不急、要请示、后厨当黑脸、惊讶得比实际重——客户拿到的价格、日期、服务都是真的，他只是在过程中觉得"我争取到了"，这是给他的情绪价值。**事实不能编**——不说"只剩一个档期"（其实空着）、"另一个客户也要这天"（没有）、"这个价今天到期"（不会）。姿态被看穿是"这人会谈"，事实被看穿是"这人骗我"，后者会变成差评。
