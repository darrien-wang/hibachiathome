...
**④ 起草。** 首条 = `认领一句 + 精确价 + 一个选择题`。展示给用户时，一条线索一个代码块，附一行"为什么这么写"。**每一条对客消息都要用户确认后才发**（用户 09-19 定，试行新销售方案期间）：先给草稿，用户说"发"才发；以前说过的"直接发/不用问我"不再适用，每条单独确认。网站和系统的自动短信（"Real Hibachi:" 开头）不在此列。
